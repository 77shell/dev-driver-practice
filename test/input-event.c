...
static int FLAG_stop_dev = 0;

static void* read_input_event(void *);

void
open_input_event_device()
{
	...
	for(i=0; i < EV_MAX; i++)
        ...
        if(pthread_create(&th_id, NULL, &read_input_event, NULL))
                printf("%s, create pthread failed~\n", __func__);
}


void
stop_input_event_device()
{
        FLAG_stop_dev = 1;
}
...
void
close_input_event_device()
{
        stop_input_event_device();
        int event;
        pthread_join(th_id, (void**)&event);
        close(fd);
}
